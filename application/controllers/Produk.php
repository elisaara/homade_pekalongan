...
class Produk extends CI_Controller {
	...
	{
		$this->session->set_userdata('breadcrumb','Produk');
		...
		if ($this->produk_model->read()->num_rows() > 0) {
			foreach ($this->produk_model->read()->result() as $produk) {
				$data[] = array(
					...
					'action' => '<button class="btn btn-sm btn-success" onclick="edit('.$produk->id.')"><i class="fa fa-edit"></i></button> <button class="btn btn-sm btn-danger" onclick="remove('.$produk->id.')"><i class="fa fa-trash"></i></button>'
				);
			}
		} else {
			$data = array();
