...
class Pelanggan extends CI_Controller {
	...
	{
		$this->session->set_userdata('breadcrumb','Pelanggan');
		...
		if ($this->pelanggan_model->read()->num_rows() > 0) {
			foreach ($this->pelanggan_model->read()->result() as $pelanggan) {
				$data[] = array(
					...
					'action' => '<button class="btn btn-sm btn-success" onclick="edit('.$pelanggan->id.')"><i class="fa fa-edit"></i></button> <button class="btn btn-sm btn-danger" onclick="remove('.$pelanggan->id.')"><i class="fa fa-trash"></i></button>'
				);
			}
		} else {
			$data = array();
