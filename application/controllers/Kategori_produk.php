...
class Kategori_produk extends CI_Controller {
	...
	{
		$this->session->set_userdata('breadcrumb','Jenis Produk');
		...
		if ($this->kategori_produk_model->read()->num_rows() > 0) {
			foreach ($this->kategori_produk_model->read()->result() as $kategori_produk) {
				$data[] = array(
					...
					'action' => '<button class="btn btn-sm btn-success" onclick="edit('.$kategori_produk->id.')"><i class="fa fa-edit"></i></button> <button class="btn btn-sm btn-danger" onclick="remove('.$kategori_produk->id.')"><i class="fa fa-trash"></i></button>'
				);
			}
		} else {
			$data = array();
