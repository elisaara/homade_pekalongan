...
class Satuan_produk extends CI_Controller {
	...
	{
		$this->session->set_userdata('breadcrumb','Satuan Produk');
		...
		if ($this->satuan_produk_model->read()->num_rows() > 0) {
			foreach ($this->satuan_produk_model->read()->result() as $satuan_produk) {
				$data[] = array(
					...
					'action' => '<button class="btn btn-sm btn-success" onclick="edit('.$satuan_produk->id.')"><i class="fa fa-edit"></i></button> <button class="btn btn-sm btn-danger" onclick="remove('.$satuan_produk->id.')"><i class="fa fa-trash"></i></button>'
				);
			}
		} else {
			$data = array();
